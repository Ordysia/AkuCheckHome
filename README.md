...
4. **Pomóż sobie** — uporządkowane edukacyjne wskazówki wybrane na podstawie zapisanych obserwacji.

**Historia** pozwala porównywać wpisy i otwierać wybrany dzień poza prowadzonym przepływem.

Skale check-inu nie mają domyślnych odpowiedzi — użytkownik świadomie wybiera każdą ocenę. Datę wpisu można wybrać w nagłówku, przy czym aplikacja nie pozwala tworzyć wpisów z przyszłości. Nawigacja obejmuje trzy główne obszary: Dzisiaj, Historia i Więcej. „Dzisiaj” prowadzi przez cztery kroki jednego wpisu, a reguły, informacje o danych oraz konto są dostępne w obszarze „Więcej”.
...
Supabase obsługuje uwierzytelnianie oraz prywatną synchronizację wpisów użytkownika.
Wpisy samopoczucia, check-iny i wyniki pulsów są zapisywane w Supabase oraz lokalnie
w `localStorage` bieżącej przeglądarki. Starszy, wspólny
...
- są dostępne po zalogowaniu na to samo konto w przeglądarce i na telefonie;
- są chronione politykami RLS i dostępne wyłącznie właścicielowi konta;
- pozostają także w lokalnej kopii, a po odzyskaniu połączenia mogą zostać ponownie zsynchronizowane.
...
Wersja pakietu: **1.2.1**. Stabilnym punktem odniesienia dla wydania 1.2 pozostaje tag [`v1.2`](https://github.com/Ordysia/AkuCheckHome/tree/v1.2).
