...
import { mergeHealthStores } from "../lib/health-data/sync";
...
test("sync merge keeps the newest independently saved section", () => {
  const local = {
    checkins: {},
    pulses: { "2026-08-02": { values: { HT: 1 }, savedAt: "2026-08-02T10:00:00.000Z" } },
    wellbeing: { "2026-08-02": { text: "lokalnie", savedAt: "2026-08-02T12:00:00.000Z" } },
  };
  const remote = {
    checkins: {},
    pulses: { "2026-08-02": { values: { HT: -1 }, savedAt: "2026-08-02T11:00:00.000Z" } },
    wellbeing: { "2026-08-02": { text: "w chmurze", savedAt: "2026-08-02T09:00:00.000Z" } },
  };
  const merged = mergeHealthStores(local, remote);
  assert.equal(merged.pulses["2026-08-02"].values.HT, -1);
  assert.equal(merged.wellbeing?.["2026-08-02"].text, "lokalnie");
});
